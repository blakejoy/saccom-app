...
const accommodationsRelations = relations(accommodations, ({ many }) => ({
...
const templateAccommodations = sqliteTable("template_accommodations", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
const templateAccommodationsRelations = relations(templateAccommodations, ({ one }) => ({
...
const formsRelations = relations(forms, ({ one, many }) => ({
  student: one(students, {
...
const formAccommodations = sqliteTable("form_accommodations", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  formId: integer("form_id").notNull().references(() => forms.id, { onDelete: "cascade" }),
  accommodationId: integer("accommodation_id").notNull().references(() => accommodations.id),
  createdAt: integer("created_at", { mode: "timestamp" }).$defaultFn(() => /* @__PURE__ */ new Date()).notNull()
});
const formAccommodationsRelations = relations(formAccommodations, ({ one, many }) => ({
  form: one(forms, {
...
const dailyTracking = sqliteTable("daily_tracking", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  formAccommodationId: integer("form_accommodation_id").notNull().references(() => formAccommodations.id, { onDelete: "cascade" }),
  dayOfWeek: integer("day_of_week").notNull(),
  // 1-5 (Mon-Fri)
  status: text("status", { enum: ["accepted", "rejected", "n/a"] }).notNull().default("n/a"),
  notes: text("notes"),
  createdAt: integer("created_at", { mode: "timestamp" }).$defaultFn(() => /* @__PURE__ */ new Date()).notNull(),
  updatedAt: integer("updated_at", { mode: "timestamp" }).$defaultFn(() => /* @__PURE__ */ new Date()).$onUpdateFn(() => /* @__PURE__ */ new Date()).notNull()
});
const dailyTrackingRelations = relations(dailyTracking, ({ one }) => ({
...
const schema = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  dailyTracking,
  dailyTrackingRelations,
  formAccommodations,
  formAccommodationsRelations,
  forms,
  formsRelations,
  students,
  studentsRelations,
  templateAccommodations,
  templateAccommodationsRelations,
  templates,
  templatesRelations
}, Symbol.toStringTag, { value: "Module" }));
const dbPath = path.join(electron.app.getPath("userData"), "sqlite.db");
...
async function initializeDatabase() {
  try {
    ...
    if (isFirstRun) {
      ...
      const migrationsFolder = electron.app.isPackaged ? path.join(process.resourcesPath, "drizzle") : path.join(electron.app.getAppPath(), "drizzle");
...
async function createStudent(event, data) {
...
async function getStudents(event, params) {
  if (params.search) {
    return await db.query.students.findMany({
      ...
      orderBy: (students2, { desc: desc2 }) => [desc2(students2.createdAt)]
    });
  }
  return await db.query.students.findMany({
    orderBy: (students2, { desc: desc2 }) => [desc2(students2.createdAt)]
...
async function getStudentById(event, params) {
  return await db.query.students.findFirst({
    where: eq(students.id, params.id),
    with: {
      forms: {
        orderBy: (forms2, { desc: desc2 }) => [desc2(forms2.year), desc2(forms2.weekNumber)],
        limit: 20
      },
      templates: true
    }
...
async function deleteStudent(event, params) {
...
function getNextWeek(weekNumber, year) {
  if (weekNumber >= 52) {
...
async function createForm(event, data) {
  const transaction = sqlite.transaction(() => {
    ...
    if (!data.isSas && data.accommodationIds && data.accommodationIds.length > 0) {
      ...
      const trackingRecords = formAccs.flatMap(
        (fa) => [1, 2, 3, 4, 5].map((day) => ({
...
async function getFormById(event, params) {
    ...
    with: {
      ...
      formAccommodations: {
        with: {
          accommodation: true,
          dailyTracking: {
            orderBy: (dailyTracking2, { asc: asc2 }) => [asc2(dailyTracking2.dayOfWeek)]
          }
        }
      }
    }
...
async function deleteForm(event, params) {
  await db.delete(forms).where(eq(forms.id, params.id));
}
async function duplicateForm(event, params) {
...
async function updateStatus(event, data) {
  await db.update(dailyTracking).set({
    status: data.status,
    notes: data.notes,
...
async function createTemplate(event, data) {
  const result = sqlite.transaction(() => {
    if (data.isDefault) {
      db.update(templates).set({ isDefault: false }).where(eq(templates.studentId, data.studentId)).run();
    }
    const templateResult = db.insert(templates).values({
      studentId: data.studentId,
      templateName: data.templateName,
      isDefault: data.isDefault
    }).returning().get();
    if (data.accommodationIds.length > 0) {
      db.insert(templateAccommodations).values(
...
async function getTemplatesByStudent(event, params) {
  return await db.query.templates.findMany({
    ...
    orderBy: (templates2, { desc: desc2, asc: asc2 }) => [
      desc2(templates2.isDefault),
      asc2(templates2.templateName)
    ]
  });
}
async function getTemplateById(event, params) {
...
async function deleteTemplate(event, params) {
  await db.delete(templates).where(eq(templates.id, params.id));
}
async function setDefaultTemplate(event, params) {
  sqlite.transaction(() => {
    db.update(templates).set({ isDefault: false }).where(eq(templates.studentId, params.studentId)).run();
    db.update(templates).set({ isDefault: true }).where(and(eq(templates.id, params.templateId), eq(templates.studentId, params.studentId))).run();
  });
}
async function getAllAccommodations(event, params) {
  return await db.query.accommodations.findMany({
    where: (accommodations2, { eq: eq2 }) => eq2(accommodations2.isActive, true),
    orderBy: (accommodations2, { asc: asc2 }) => [asc2(accommodations2.sortOrder)]
  });
}
const isDev$1 = process.env.NODE_ENV === "development";
async function openWindow(event, params) {
  const childWindow = new electron.BrowserWindow({
    width: params.width || 1e3,
  ...
  const baseURL = isDev$1 ? "http://localhost:5173" : `file://${path.join(__dirname, "../../dist-renderer/index.html")}`;
  const fullURL = isDev$1 ? `${baseURL}#${params.url}` : `${baseURL}#${params.url}`;
...
function registerIpcHandlers() {
  ...
  electron.ipcMain.handle("students:create", createStudent);
  electron.ipcMain.handle("students:getAll", getStudents);
  electron.ipcMain.handle("students:getById", getStudentById);
  electron.ipcMain.handle("students:delete", deleteStudent);
  electron.ipcMain.handle("forms:create", createForm);
  electron.ipcMain.handle("forms:getById", getFormById);
  electron.ipcMain.handle("forms:delete", deleteForm);
  electron.ipcMain.handle("forms:duplicate", duplicateForm);
  electron.ipcMain.handle("tracking:updateStatus", updateStatus);
  electron.ipcMain.handle("templates:create", createTemplate);
  electron.ipcMain.handle("templates:getByStudent", getTemplatesByStudent);
  electron.ipcMain.handle("templates:getById", getTemplateById);
  electron.ipcMain.handle("templates:delete", deleteTemplate);
  electron.ipcMain.handle("templates:setDefault", setDefaultTemplate);
  electron.ipcMain.handle("accommodations:getAll", getAllAccommodations);
  electron.ipcMain.handle("window:open", openWindow);
...
function createWindow() {
  ...
  const startURL = isDev ? "http://localhost:5173" : `file://${path.join(__dirname, "../dist-renderer/index.html")}`;
